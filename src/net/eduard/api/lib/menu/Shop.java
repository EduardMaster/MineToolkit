...
public class Shop extends Menu {
    ...
    public static String MESSAGE_ALREADY_BOUGHT = "§aJá adiquirido";
    ...
    private transient Map<Player, Product> confirmingTransaction = new HashMap<>();
    ...
    private String messageAlreadyBought = MESSAGE_ALREADY_BOUGHT;
    ...
    public void useConfirmationMenu() {
        confirmation = new Menu("Confirmar Transação", 3);
        confirmation.setSuperiorMenu(this);
        confirmation.register(getPlugin());
        MenuButton confirmationButton = new MenuButton("confirmar", confirmation);
        confirmationButton.setPosition(3, 2);
        confirmationButton.setIcon(new ItemBuilder(Material.WOOL).data(5).name("§a§lCONFIRMAR").lore("§aClique para confirmar a transação."));
        MenuButton cancelButton = new MenuButton("cancelar", confirmation);
        cancelButton.setIcon(new ItemBuilder(Material.WOOL).data(14).name("§c§lCANCELAR").lore("§cClique para cancelar a transação."));
        cancelButton.setPosition(7, 2);
        cancelButton.setClick((event, page) -> {
            open((Player) event.getWhoClicked(), page);
        });
        MenuButton productButton = new MenuButton("product", confirmation);

        productButton.setPosition(5, 2);
        productButton.setIcon(new ItemBuilder(Material.STONE));

        confirmationButton.setClick((event, page) -> {
            if (event.getWhoClicked() instanceof Player) {
                Player player = (Player) event.getWhoClicked();
                Product produto = confirmingTransaction.get(player);
                TradeType type = trading.get(player);
                if (type == TradeType.BUYABLE) {
                    buy(player, produto, 1);
                } else if (type == TradeType.SELABLE) {
                    sell(player, produto, 1);
                }
            }
        });
    }
        ...
        if (e.getPlayer() instanceof Player) {
            ...
            if (confirmation != null) {
                if (confirmation.isOpen(player)) {
                    MenuButton productButton = confirmation.getButton("product");
                    Product product = confirmingTransaction.get(player);
                    e.getInventory().setItem(productButton.getIndex(), product.getIcon());
                }
            }
        }
            ...
            setEffect((event, page) -> {
                if (event.getWhoClicked() instanceof Player) {
                    ...
                    if (confirmation != null) {
                        ...
                        trading.put(player, product.getTradeType());
                        confirmingTransaction.put(player, product);
...
//
        ...
        public Map<Player, Product> getConfirmingTransaction () {
