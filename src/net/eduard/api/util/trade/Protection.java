...
public class Protection {
	...
	public static void main(String[] args) {
		...
		if (opcao == 1) {
			...
			while (true) {
				try {
					Protection venda = new Protection();
	...
	public static List<Protection> getVendas(String url) {
		List<Protection> vendas = new ArrayList<>();
		try {
			...
			while (scan.hasNext()) {
				...
				vendas.add(new Protection(line));
			}
			scan.close();
...
//			e.printStackTrace();
	...
	public static void checkOnline(JavaPlugin plugin, String url) {
		...
		List<Protection> keys = getVendas(url);
		if (keys == null) {
