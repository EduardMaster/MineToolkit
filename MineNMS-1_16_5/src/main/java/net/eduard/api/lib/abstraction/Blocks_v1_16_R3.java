...
public class Blocks_v1_16_R3 extends CraftBlock implements Blocks {
    ...
    public Blocks_v1_16_R3(int x, int y, int z, WorldServer worldServer) {
        super(null, new BlockPosition(x,y,z));
        this.worldServer = worldServer;

    }
    public Blocks_v1_16_R3(Location location)
    {
        this(location.getBlock());
    }
    public Blocks_v1_16_R3(org.bukkit.block.Block block) {
    ...
    public int getTypeId() {
        ...
        // Block.getId(block);
        return Block.getCombinedId(type);
    }
    ...
    public org.bukkit.block.Block getRelative(int modX, int modY, int modZ) {
        return new Blocks_v1_16_R3(getX() + modX, getY() + modY, getZ() + modZ, worldServer);
    }

    private ChunkSection getSection() {
        final Chunk chunk = worldServer.getChunkProvider()
                .getChunkUnchecked(getX() >> 4, getZ() >> 4);

        ChunkSection chunkSection = chunk.getSections()[getY() >> 4];
        if (chunkSection == null) {
            chunkSection
                    = chunk.getSections()[getY() >> 4]
                    = new ChunkSection(getY() >> 4 << 4);
        }
