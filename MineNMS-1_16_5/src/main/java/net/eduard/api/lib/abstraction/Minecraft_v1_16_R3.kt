...
class Minecraft_v1_16_R3 : Minecraft() {
    ...
    override fun sendParticle(
        ...
        val particula: Particles = result as Particles
        /*
        if (particula.f()) {
    ...
    override fun reloadPlayer(player: Player) {
        ...
        (Bukkit.getServer() as CraftServer).handle.moveToWorld(
            player.handle, player
                .handle.world.minecraftWorld, true, player.getLocation(),
            true
        )
    }
    ...
    override fun getItemNBT(item: ItemStack): Any {
        val nmsItem: net.minecraft.server.v1_16_R3.ItemStack = CraftItemStack.asNMSCopy(item)
        val nbt: NBTTagCompound = if (nmsItem.tag == null) nmsItem.tag!!
        else NBTTagCompound()
        return nbt
    }
    ...
    init {
        /*
        if (particles.isEmpty()) for (particle in Particles.values()) {
